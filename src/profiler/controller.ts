...
module Tools.Profiler {
  ...
  export class Controller {
    ...
    private _overviewHeader: ScrubberChart;
    private _overview: FlameChartOverview;
    // private _headers: ScrubberChart [];
    private _charts: FlameChart [];
    // _functionCharts: FunctionChart [];
    ...
    private _createViews() {
      if (this._activeProfile) {
        var self = this;
        this._overviewHeader = new Profiler.ScrubberChart(this, ScrubberChartType.OVERVIEW);
        ...
        this._activeProfile.forEachSnapshot(function (snapshot:TimelineBufferSnapshot, index:number) {
          // self._headers.push(new Profiler.ScrubberChart(self, ScrubberChartType.OVERVIEW));
          self._charts.push(new Profiler.FlameChart(self, snapshot));
        });
        //this._functionCharts.push(new Profiler.FunctionChart(this, FlameChartOverviewMode.OVERLAY));
        //this._functionCharts.push(new Profiler.FunctionChart(this, FlameChartOverviewMode.OVERLAY));
        //this._functionCharts.push(new Profiler.FunctionChart(this, FlameChartOverviewMode.OVERLAY));
        //this._functionCharts.push(new Profiler.FunctionChart(this, FlameChartOverviewMode.OVERLAY));
    ...
    private _destroyViews() {
      if (this._activeProfile) {
        ...
        //while (this._functionCharts.length) {
        //  this._functionCharts.pop().destroy();
        //}
    ...
    private _initializeViews() {
      if (this._activeProfile) {
        ...
        this._activeProfile.forEachSnapshot(function (snapshot:TimelineBufferSnapshot, index:number) {
          ...
          self._charts[index].initialize(startTime, endTime, 256);
        });
        //this._functionCharts.forEach(function (chart:ChartBase, index:number) {
        //  chart.initialize(startTime, endTime);
        //});
      }
    }
