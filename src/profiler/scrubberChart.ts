...
module Tools.Profiler {
  ...
  export class ScrubberChart extends ChartBase implements MouseControllerTarget {

    private _type: ScrubberChartType;

    private static TICK_MAX_WIDTH = 75;

    constructor(controller: Controller, type: ScrubberChartType) {
      this._type = type;
      super(controller);
    }
    ...
    private _drawDragHandle(pos: number) {
      ...
      this._drawRoundedRect(context, pos - ChartBase.DRAGHANDLE_WIDTH / 2, 1, ChartBase.DRAGHANDLE_WIDTH, this._height - 2, 2, true);
    }
    ...
    _toPixels(time: number): number {
      var start = (this._type === ScrubberChartType.OVERVIEW) ? this._rangeStart : this._windowStart;
